...
2) It is assumed that the command line version of the GCC compiler is installed
   ...
   I have tested the port with several versions of avr-gcc from the
   Winavr distribution (versions released in 2003) and also with Linux
   versions of the same tools (rpm distributions provided in
   http://savannah.nongnu.org/download/avr-libc/released/ or
...
Micrium/Software/uCOS-II/Ports/AVR/ATmega128/GNU/jlu/STK500
       This directory contains two sample test codes along with
       ...
       marked as being for the STK500 development board, it does not have much
       that is tied up to the STK500 board.

Micrium/Software/uCOS-II/Ports/AVR/ATmega128/GNU/jlu/STK500/ex1
	The test code originally from Jesper Hansen's port with modifications.	

Micrium/Software/uCOS-II/Ports/AVR/ATmega128/GNU/jlu/STK500/ex2 
        A more simple test code with only three tasks so that the code should
	run with AVRs that have at least 1 kB of SRAM.
...
3) Although the port is labeled for ATmega128, I have tried my best to
   support other AVR processors. I have tested my test codes with
   - ATmega128 with STK-500+STK-501 boards with and without external memory.
   - ATmega163 with STK-500 without external memory (only ex2)
   I have had several user reports from users who have used other AVRs e.g.
   ATmega32.
...
4) TIMER0 overflow interrupt is used to generate the clock tick (see
   ...
   in os_cfg.h (see the example projects). Depending on the
   processor, the example projects set the TCCR0 value to 0x07 or
   0x05 in order to set the clock divider to 1024.
...
7) You will also need the source code for uC/OS-II to compile this
   ...
   Labrosse's book "MicroC/OS-II, The Real Time Kernel" (2nd
   edition). More information about uC/OS-II can be found at
   www.ucos-ii.com or www.micrium.com. Please don't ask me to send the
   source to you, I won't do that.

8) If you have uC/OS-II v2.52, see the README files of the example projects.
...
October 14, 2003: v1.21
-----------------------

 1) Corrected the following bugs:

 os_cpu_c.c: 

 - bug correction #1: now compiles if OS_TASK_SW_HOOK_EN is not
   defined (when using OS_VERSION < 260) 
 - bug correction #2: added conditional compilation around
   OSTaskStatHook (OS_CPU_HOOKS_EN > 0)

 os_cpu_a.asm:

  - now all subroutine calls use 'rcall' with devices with <= 8 kB of
    program memory (actually call should be an allowable opcode with
    most of the MCUs, but avr-as doesn't allow this for some
    reason. Therefore the compilation failed with some MCUs).

 avr_ish.h

  - pushregs() now clears r1 ("zero register" in gcc) as gcc does with
    SIGNAL() and INTERRUPT() (Bug reported by cpqxl <cpqxl@katamail.com>)

 2) Added a simple test program in directory STK500/ex2. This test
    program needs a little less than 1 kB of SRAM, so it fits in
    devices with 1 kB of SRAM. The original test program is now in STK500/ex1.
