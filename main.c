...
#   define SIG_UART1_DATA SIG_UART_DATA 
#endif					
...
int main (void)
{
	...
	OSInit();	/* creates the idle task and start task */

	BoardInit();
	
	OSTaskCreate(TaskMain, NULL, (void *)&TaskStk[0][TASK_STK_SIZE - 1], 0);   
...
void TaskMain (void *data)
{
  ...
  INT8U err;
  /* 
   * Timer interupt is enabled here, not before OSStart()!
   */
  ...
  OS_EXIT_CRITICAL();
	...
	ks0108GotoXY(0,0);
	ks0108Puts_P(PSTR("uCOSii"));
	ks0108GotoXY(0,16);
	ks0108Puts_P(PSTR("Running..."));

	OSTaskCreate(TaskGsm, (void *)0, (void *)&TaskStk[1][TASK_STK_SIZE - 1], 3);   
	while(1)
	{
...
TagAtCmdDetails AtCmdDetails;

void TaskGsm(void *data)
{
	TagAtCmdResp CmdResp;
	
	AtCmdDetails.uint8CmdPtr = "AT\r\n";
	AtCmdDetails.uint16CmdTmout = 10;
	AtCmdDetails.CmdRsp = GSM_CHK_OK;

	while(1)
	{
		SendAtCommand(&AtCmdDetails, YES, &CmdResp);

		if(CmdResp EQ GSM_RSP_OK)
		{
			LED_DBG ^= LED_ON;
		}
		else if(CmdResp EQ GSM_RSP_TMOUT)
		{
			ks0108GotoXY(0,48);
			ks0108Puts_P(PSTR("Timeout"));
		}
		else
		{
			ks0108GotoXY(0,48);
			ks0108Puts_P(PSTR("KNA Bug"));
		}
       ...
       for(i=0; i<bytes; i++)
       {
               ...
               for(j=0; j<width; j++)
               {
                       ...
                       if (i + 1 == bytes)
                       {
                               ...
                               uint8_t dataOld = ks0108FontRead(ks0108Font+index+pageOld+j); //get last byte
                               dataOld >>= tooMuch;
                               data |= dataOld;
                       }
